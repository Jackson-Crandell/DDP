...
Q_f = zeros(2,2);
Q_f(1,1) = 10000; 	%Penalize more w.r.t errors in theta
Q_f(2,2) = 100;      %Penalize more w.r.t errors in theta_dot
...
% Best: 1
R = .01* eye(1,1); % Weight control equally
...
sigma_vec = [0.0, 0.01, 0.015, 0.02, 0.1]
x_traj_noise = zeros(2,Horizon,length(sigma_vec));
cost_noise = zeros(1,num_iter);
...
[x_traj] = simulate(xo,u_new,Horizon,dt,sigma_vec(kk));   %Create new nominal trajectory based on new control (u_new)
...
plot(time,x_traj_noise(1,:,1),'linewidth',4); 
plot(time,x_traj_noise(1,:,2),'linewidth',4); 
plot(time,x_traj_noise(1,:,3),'linewidth',4); 
plot(time,x_traj_noise(1,:,4),'linewidth',4); 
plot(time,x_traj_noise(1,:,5),'linewidth',4); 
title('$\theta$','Interpreter','latex','fontsize',24)
xlabel('Time in sec','fontsize',20)
legend('Target','$\sigma$ = 0.0', '$\sigma$ = 0.01', '$\sigma$ = 0.015','$\sigma$ = 0.02','$\sigma$ = 0.1','Interpreter','latex', 'fontsize',24)
%hold off;
grid;
...
plot(time,x_traj_noise(2,:,1),'linewidth',4); 
plot(time,x_traj_noise(2,:,2),'linewidth',4); 
plot(time,x_traj_noise(2,:,3),'linewidth',4); 
plot(time,x_traj_noise(2,:,4),'linewidth',4); 
plot(time,x_traj_noise(2,:,5),'linewidth',4);
...
plot(cost_noise(:,:,1),'linewidth',4); 
plot(cost_noise(:,:,2),'linewidth',4); 
plot(cost_noise(:,:,3),'linewidth',4); 
plot(cost_noise(:,:,4),'linewidth',4); 
plot(cost_noise(:,:,5),'linewidth',4); 
xlabel('Iterations','fontsize',20)
title('Cost','fontsize',20);
legend('$\sigma$ = 0.0', '$\sigma$ = 0.01', '$\sigma$ = 0.015','$\sigma$ = 0.02','$\sigma$ = 0.1','Interpreter','latex', 'fontsize',24)
